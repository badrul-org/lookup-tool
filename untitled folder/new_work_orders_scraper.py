...
async def update_fieldedge_wo_description(fe_page, wo_number, result_link, full_wo_url=None):
    """
    Navigates to FieldEdge Dispatch Summary page https://login.fieldedge.com/#/DispatchSummary/<d_id>/<dt_id>,
    locates the description box (//textarea[@placeholder="Type Here"])[1], prepends the result_link,
    clicks Save, confirms via #confirm-save-dispatch-summary, and waits 10s.
    """
    try:
        print(f"Updating description in FieldEdge for WO {wo_number}...")
        
        # 1. Navigate back to Dispatch Summary page if currently on Customer page
        target_url = full_wo_url if full_wo_url else f"https://login.fieldedge.com/#/DispatchSummary/{wo_number}"
        if target_url and target_url not in fe_page.url:
            print(f"Navigating to Dispatch Summary page: {target_url}")
            try:
                await fe_page.goto(target_url, wait_until="networkidle")
                await fe_page.wait_for_timeout(3000)
                await dismiss_fieldedge_popup(fe_page)
            except Exception as ne:
                print(f"Navigation warning to {target_url}: {ne}")

        # 2. Locate description textarea using exact placeholder selector
        desc_textarea = fe_page.locator('(//textarea[@placeholder="Type Here"])[2]').first
        if not await desc_textarea.is_visible(timeout=3000):
            try:
                await desc_textarea.wait_for(state="visible", timeout=5000)
            except Exception:
                pass

        if await desc_textarea.is_visible():
            existing_desc = await desc_textarea.input_value()
            if result_link not in existing_desc:
                new_desc = f"Scanned Reports: {result_link}\n\n{existing_desc}".strip()
                await desc_textarea.fill(new_desc)
                await fe_page.wait_for_timeout(1000)

                # Click Save button
                save_btn = fe_page.locator("//button[contains(@class,'confirm')]//span[normalize-space()='Save'] | //span[normalize-space()='Save'] | //button[normalize-space()='Save']").first
                if await save_btn.is_visible(timeout=5000):
                    await save_btn.click()
                    await fe_page.wait_for_timeout(2000)
                    print(f"Clicked Save for WO {wo_number} description.")

                # Click confirm button: //button[@id="confirm-save-dispatch-summary"]
                confirm_btn = fe_page.locator('//button[@id="confirm-save-dispatch-summary"]').first
                try:
                    if await confirm_btn.is_visible(timeout=5000):
                        await confirm_btn.click()
                        print(f"Clicked confirm button (#confirm-save-dispatch-summary) for WO {wo_number}.")
                except Exception as ce:
                    print(f"Note on confirm button (#confirm-save-dispatch-summary): {ce}")

                # Wait 10s as instructed
                print("Waiting 10s after saving description...")
                await fe_page.wait_for_timeout(10000)
                print(f"✅ Successfully updated Work Order {wo_number} description with result link!")
            else:
                print(f"Result link already present in description for WO {wo_number}.")
        else:
            print(f"⚠️ Description box not visible on page for WO {wo_number}.")
    except Exception as e:
        print(f"⚠️ Could not update FieldEdge WO description for {wo_number}: {e}")
...
def init_work_orders_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS work_order_reports (
            ...
            address TEXT,
            location_code TEXT,
            tax_code_status TEXT,
            customer_tax_code_status TEXT,
            rme_urls TEXT DEFAULT '[]',
            tpchd_urls TEXT DEFAULT '[]',
            king_urls TEXT DEFAULT '[]',
            accella_urls TEXT DEFAULT '[]',
    ...
    for i in range(row_count):
        ...
        if db_cursor.fetchone():
            ...
            try:
                ...
                # 3. King County Reports
                if king_result:
                    king_status = "done"

                # 4. Copy Accella Reports to Persistent Local Storage
                accella_urls_web = []
                persistent_dir = os.path.join("static", "uploads", "wo_pdfs", str(wo_number))
                ...
                for item in accella_result:
                    ...
                    if os.path.exists(src_path):
                        ...
                        target_filepath = os.path.join(persistent_dir, target_filename)
                        counter = 1
                        while os.path.exists(target_filepath):
                            target_filename = f"{base_name}_{counter}.pdf"
                            target_filepath = os.path.join(persistent_dir, target_filename)
                            counter += 1
                        
                        try:
                            shutil.copy2(src_path, target_filepath)
                            web_path = f"/static/uploads/wo_pdfs/{wo_number}/{target_filename}"
                            accella_urls_web.append(web_path)
                            os.remove(src_path)
                        except Exception as e:
                            print(f"Failed to copy Accella PDF: {e}")
                            accella_status = "error"
                    else:
                        accella_status = "error"

                # 5. Fetch Customer Invoice & Update FieldEdge Description with Result Link
                invoice_url_web = ""
                result_link = f"https://dashboard.sterlingsepticandplumbing.com/wo-result/{wo_number}"

                print(f"Processing Customer Invoice & FieldEdge update for WO {wo_number}...")
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(f"Failed to fetch location code: {e}")
                        finally:
                        ...
                        if location_code:
                            ...
                            try:
                                ...
                                print(f"Tax code update failed: {e}")
                                tax_code_status = "error"

                        if href:
                            cust_url = f"https://login.fieldedge.com/{href}"
                            try: await fe_page.goto(cust_url, wait_until="networkidle")
                            except: pass
                            await fe_page.wait_for_timeout(3000)
                            await dismiss_fieldedge_popup(fe_page)
                            if location_code_val != "not found":
                                try:
                                    edit_res = await edit_customer_and_update_tax_code(fe_page, location_code_val)
                                    customer_tax_code_status = "done" if edit_res == "updated" else ("already updated" if edit_res == "already updated" else "error")
                                except: customer_tax_code_status = "error"
                            ...
                            if invoice_pdf_url:
                                ...
                                if r.status_code == 200:
                                    ...
                                    inv_filename = f"INVOICE_{invoice_date}.pdf"
                                    inv_filepath = os.path.join(persistent_dir, inv_filename)
                                    counter = 1
                                    while os.path.exists(inv_filepath):
                                        inv_filename = f"INVOICE_{invoice_date}_{counter}.pdf"
                                        inv_filepath = os.path.join(persistent_dir, inv_filename)
                                        counter += 1
                                    with open(inv_filepath, 'wb') as f:
                                        for chunk in r.iter_content(chunk_size=8192): f.write(chunk)
                                    if os.path.getsize(inv_filepath) > 1000:
                                        invoice_url_web = f"/static/uploads/wo_pdfs/{wo_number}/{inv_filename}"
                                    else: invoice_status = "error"
                                else: invoice_status = "error"

                        await update_fieldedge_wo_description(fe_page, wo_number, result_link, full_wo_url)
                    except Exception as e:
                        print(f"⚠️ Failed to process invoice/description for WO {wo_number}: {e}")
                        error_details = f"Processing error: {str(e)}"
                except Exception as e:
                    print(f"⚠️ Failed to handle WO {wo_number}: {e}")
                    error_details = str(e)
                    try:
            ...
            if restart_needed: break

            print(f"WO {wo_number} successfully finished! Saving results to DB.")
            if final_rme_urls and rme_status != "error": rme_status = "done"
            if tpchd_result and tpchd_status != "error": tpchd_status = "done"
            if king_result and king_status != "error": king_status = "done"
            if accella_urls_web and accella_status != "error": accella_status = "done"
            if invoice_url_web and invoice_status != "error": invoice_status = "done"

            db_cursor.execute('INSERT INTO work_orders (wo_number, address, error_message, rme_status, tpchd_status, king_status, accella_status, invoice_status, run_time, location_code, tax_code_status, customer_tax_code_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                (wo_number, address, error_details, rme_status, tpchd_status, king_status, accella_status, invoice_status, run_time, location_code_val, tax_code_status, customer_tax_code_status))

            db_cursor.execute('''INSERT OR REPLACE INTO work_order_reports (wo_number, address, location_code, tax_code_status, customer_tax_code_status, rme_urls, tpchd_urls, king_urls, accella_urls, invoice_url, result_link) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                (str(wo_number), address, location_code_val, tax_code_status, customer_tax_code_status, json.dumps(final_rme_urls), json.dumps(tpchd_result), json.dumps(king_result), json.dumps(accella_urls_web), invoice_url_web, result_link))
            db_conn.commit()
